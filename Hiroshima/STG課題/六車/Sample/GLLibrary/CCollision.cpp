...
void CAABB::Draw(CVector4D& color) {
	glPushMatrix();
...
bool CCollision::CompareLength(const COBB &A,const COBB &B,const CVector3D &distVec,const CVector3D &separate,float *length){
	...
	float distA = 0;
	for(int i=0;i<3;i++) distA+=fabsf( CVector3D::Dot(A.m_axis[i],separate)) * A.m_length[i];
	float distB = 0;
	for(int i=0;i<3;i++) distB+=fabsf( CVector3D::Dot(B.m_axis[i],separate)) * B.m_length[i];
...
bool CCollision::CollisionOBBShpere(const COBB &A,const CVector3D &center , float radius){
	...
	CVector3D VL;
	for(int i=0;i<3;i++) {
		VL.v[i] = fabsf(CVector3D::Dot(A.m_axis[i],V)) - A.m_length[i];
