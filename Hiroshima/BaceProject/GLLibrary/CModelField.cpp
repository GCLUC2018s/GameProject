...
CFieldPoly::CFieldPoly() : m_pVertex(NULL), m_colmask(NULL), m_colIdx(NULL), colNormal(NULL),m_buffer(0){
...
int CFieldPoly::CollisionSphere(CCollTriangle *out, const CVector3D &center, float radius, int maxcnt, int raws, int rawe, int cols, int cole){
	if (m_vertexCnt <= 0) return false;
	SFieldVertex *v = m_pVertex;
	memset(m_colmask, 0, sizeof(unsigned long)*((m_vertexCnt / 3 / 32) + 1));
...
int CFieldPoly::CollisionCupsel(CCollTriangle *out, const CVector3D &top, const CVector3D &bottom, float radius, int maxcnt, int raws, int rawe, int cols, int cole){
	if (m_vertexCnt <= 0) return false;
	int cnt = 0;
	SFieldVertex *v = m_pVertex;
	memset(m_colmask, 0, sizeof(unsigned long)*((m_vertexCnt / 3 / 32) + 1));
...
bool CFieldMesh::CollisionRay(CVector3D *c, CVector3D *n, const CVector3D &s, const CVector3D &e, float *pLength, const SFieldModelHeader &header){
...
int CFieldMesh::CollisionSphere(CCollTriangle *out, const CVector3D &center, float radius, int maxcnt, const SFieldModelHeader &header) {
	...
	int cnt = 0;
	for (unsigned int i = 0; i < m_polyCnt; i++) {
		int c = m_poly[i].CollisionSphere(out, center, radius, maxcnt, raws, rawe, cols, cole);
		out += c;
		cnt += c;
		if (cnt >= maxcnt) break;
	}
...
int CFieldMesh::CollisionCupsel(CCollTriangle *out, const CVector3D &top, const  CVector3D &bottom, float radius, int maxcnt, const SFieldModelHeader &header){
	...
	for (unsigned int i = 0; i < m_polyCnt; i++) {
		int c = m_poly[i].CollisionCupsel(out, top, bottom, radius, maxcnt, raws, rawe, cols, cole);
		out += c;
		cnt += c;
		if (cnt >= maxcnt) break;
	}
...
void CModelField::CalcRenderRange() {
	float showLength = m_length / m_scale.z;

	m_center = m_matrix.GetInverse() * CCamera::GetCurrent()->GetPos();
...
void CModelField::RenderM(CMatrix &m){
	m_isRender = true;
...
bool CModelField::CollisionRay(CVector3D *pCross, CVector3D *pNormal, const CVector3D &s, const CVector3D &e){
	...
	for (int x = minX; x <= maxX; x++) {
		for (int z = minZ; z <= maxZ; z++) {
			int idx = GetIdx(x, z);
			if (m_pMesh[idx].CollisionRay(&c, &n, s2, e2, &length,m_header))
				hit = true;	
		}
	}
	if (hit){
...
int CModelField::CollisionSphere(CCollTriangle *out, const CVector3D &center, float radius, int maxcnt){
...
int CModelField::CollisionCupsel(CCollTriangle *out, const CVector3D &top, const CVector3D &bottom, float radius, int maxcnt){
