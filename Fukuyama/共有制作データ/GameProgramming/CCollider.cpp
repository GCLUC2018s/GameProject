...
bool C_Collider::Collision(C_Object* ip_MyCol, C_Object* ip_YouCol, C_Vector3* ip_MyShadowCol, C_Vector3* ip_YouShadowCol){
	if (ip_MyCol->m_Colimage.m_Enabled&&ip_YouCol->m_Colimage.m_Enabled){
		...
		if (fabs(ip_MyShadowCol->z - ip_YouShadowCol->z) > ip_MyCol->m_Collision_Wide + ip_YouCol->m_Collision_Wide){
			//returnで返す
			return false;
		}
		else{
			...
			//z軸の判定
				C_Collider::CCollisionZ(ip_MyCol, ip_YouCol);
			//x軸の判定
			if (ip_MyCol->m_Colimage.m_Left >= ip_YouCol->m_Colimage.m_Right ||
				...
				//左判定
				if (ip_YouCol->m_Colimage.m_Left >= ip_MyCol->m_Colimage.m_Left){
					ip_YouCol->m_Colimage.m_AdjustX = ip_MyCol->m_Colimage.m_Right - ip_YouCol->m_Colimage.m_Left;
				}
				//ip_Col1右、ip_Col2左判定の場合
				//右判定
				if (ip_YouCol->m_Colimage.m_Right <= ip_MyCol->m_Colimage.m_Right){
					ip_YouCol->m_Colimage.m_AdjustX = ip_MyCol->m_Colimage.m_Left - ip_YouCol->m_Colimage.m_Right;
				}
				return true;
			}
		}
...
bool C_Collider::CCollisionZ(C_Object *ip_Col1, const C_Object *ip_Col2){
