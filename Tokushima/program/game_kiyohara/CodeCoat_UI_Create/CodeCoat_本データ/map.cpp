...
//変更、追加
void CMapControl::loadFiles(){
	char *item[] = { "back1.png", "back2.png","goal.png"};
	char buf[256];
	for (int i = 0; i < IMAGECOUNT; i++){
		sprintf_s(buf, "media\\img\\%s", item[i]);
		m_img[i] = LoadGraph(buf);
...
void CMapControl::Update(){
	...
	if ((m_totalmovement > m_clearpos) && m_goalflag == false){
		...
		m_posAsrc.setX(m_posAsrc.getX() - Amount);	//ここで座標の移動をする
		if (m_posAsrc.getX() <= -2560)
			m_posAsrc.setX(0);
		m_posBsrc.setX(m_posBsrc.getX() - (Amount/5));
		if (m_posBsrc.getX() <= -1280)
			m_posBsrc.setX(0);
		if (m_goalflag){

			if (m_goalpos.getX() > 0){
			m_goalpos.setX(m_goalpos.getX() - Amount);
			if (m_goalpos.getX() < 0)
				m_goalpos.setX(0.0f);
		}
		}
