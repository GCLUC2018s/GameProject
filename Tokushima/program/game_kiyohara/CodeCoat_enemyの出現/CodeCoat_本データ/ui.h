...
const float MAP_P_INIT_X = 0;				//ミニマップのプレイヤーX
const float MAP_P_INIT_Y = 575;				//ミニマップのプレイヤーY
...
const float SCORE_MAGNIFICATION_X = 70;		//スコア倍率のX座標
...
private:
	...
	CVector2D m_timer_pos;		//制限時間の座標
	CVector2D m_arrow_pos;		//スコア倍率の矢印の座標
	CVector2D m_maparrow_pos;	//ミニマップの自機の座標
...
public:
	...
	void Update();
	void Draw();
	void scoreMagnification(CVector2D *pos);	//
